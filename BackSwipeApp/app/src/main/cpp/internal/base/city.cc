...
// MOE:begin_strip
//#include "util/hash/avoid_tradesecret.h"
...
namespace util_hash {
    ...
    static uint64 Rotate(uint64 val, size_t shift) {
                DCHECK_LE(shift, 63);
    ...
    static std::pair<uint64, uint64> WeakHashLen32WithSeeds(uint64 w, uint64 x,
                                                            uint64 y, uint64 z,
                                                            uint64 a, uint64 b) {
        a += w;
        b = Rotate(b + a + z, 51);
    ...
    static std::pair<uint64, uint64> WeakHashLen32WithSeeds(const char *s, uint64 a,
                                                            uint64 b) {
    ...
    uint128 CityHash128(const char *s, size_t len) {
        if (len >= 16) {
            return CityHash128WithSeed(s + 16, len - 16,
                                       MakeUint128(LittleEndian::Load64(s) ^ k3,
                                                   LittleEndian::Load64(s + 8)));
        } else if (len >= 8) {
            return CityHash128WithSeed(
                    nullptr, 0, MakeUint128(LittleEndian::Load64(s) ^ (len * k0),
                                            LittleEndian::Load64(s + len - 8) ^ k1));
        } else {
            return CityHash128WithSeed(s, len, MakeUint128(k0, k1));
        }
    }

    uint32 CityHash32(const char *s, size_t len) {
        return static_cast<uint32>(CityHash64(s, len));
    }

    uint32 CityHash32WithSeed(const char *s , size_t len, uint32 seed) {
        return static_cast<uint32>(CityHash64WithSeed(s, len, seed));
    }
...
namespace { static size_t Seed() { return HashSeed() + 91; } }

uint64 CityHash64(const char *s, size_t len) {
  return farmhash::Hash64(s, len) ^ Seed();
}

uint64 CityHash64WithSeed(const char *s, size_t len, uint64 seed) {
  return farmhash::Hash64WithSeed(s, len, seed + Seed());
}

uint64 CityHash64WithSeeds(const char *s, size_t len,
                           uint64 seed0, uint64 seed1) {
  return farmhash::Hash64WithSeeds(s, len, seed0, seed1 + Seed());
}
...
uint32 CityHash32(const char *s, size_t len) {
  return static_cast<uint32>(farmhash::Hash32(s, len) ^ Seed());
}

uint32 CityHash32WithSeed(const char *s, size_t len, uint32 seed) {
  return static_cast<uint32>(farmhash::Hash32WithSeed(
      s, len, static_cast<uint32>(seed - Seed())));
}
